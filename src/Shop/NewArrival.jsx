...
import { newArrival } from "../Data/ProductInfomation";
...
function NewArrival() {
  const productRefs = useInViewAnimation("active", 200); // 100ms delay per
  ...
  return (
    <React.Fragment>
      <div className="flex flex-col gap-4 mt-2">
        <h2 className="text-xl font-bold font-rowdies ">New Arrival</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-4">
          {newArrival.map((render, index) => {
            return (
              <div
                key={render?.id}
